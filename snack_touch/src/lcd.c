...
{
	char restart[]="按这里重新开始";
	int i,loc=124;
	lcd_flush(0xffffff);
	for(i=0;i<strlen(GameOver);++i)
		lcd_dischar_816(&szASC16[GameOver[i]*16],112,loc+i*8,0xff);
	loc=0;
	for(i=0;i<strlen(restart);)
	{
		if(i>1&&i<6)//i在2到5时，"这里"两个字用红字标明
		{
			if(restart[i]>127)
			{
		...
		else if(restart[i]>127)
		{
			lcd_dischar_1616(&szHZK16[((restart[i]-161)*94+(restart[i+1]-161))*32],128,112+loc,0xff);
			loc+=16;
			i+=2;
		}
		else
		{
			lcd_dischar_816(&szASC16[restart[i]*16],128,112+loc,0xff);
			loc+=8;
			++i;
		}
	}
...
void lcd_adjust(void)
{
	...
	switch(adjusting)
	{
		case 1:
			for(i=0;i<9;++i)
			{
				lcd_buf[30+grim[i][0]][40+grim[i][1]]=0xffffff;
			}
			break;
		case 2:
			for(i=0;i<9;++i)
			{
				lcd_buf[30+grim[i][0]][280+grim[i][1]]=0xffffff;
			}
			break;
		case 3:
			for(i=0;i<9;++i)
			{
				lcd_buf[210+grim[i][0]][280+grim[i][1]]=0xffffff;
			}
			break;
		case 4:
			for(i=0;i<9;++i)
			{
				lcd_buf[210+grim[i][0]][40+grim[i][1]]=0xffffff;
			}
			break;
	}
}
